...
-- Create unidades_familiares table
CREATE TABLE IF NOT EXISTS public.unidades_familiares (
    codigo_vivienda TEXT PRIMARY KEY,
    total_participacion NUMERIC(5, 2) NOT NULL DEFAULT 0 CHECK (total_participacion >= 0),
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT fk_unidad_familiar_vivienda FOREIGN KEY (codigo_vivienda) 
        REFERENCES public.viviendas(codigo) 
        ON DELETE RESTRICT 
        ON UPDATE CASCADE
);
...
CREATE OR REPLACE FUNCTION public.recalcular_participacion_unidad_familiar(p_codigo_vivienda TEXT)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    -- Get vivienda participation percentage
    SELECT COALESCE(porcentaje_participacion, 0) INTO v_porcentaje_vivienda
    FROM public.viviendas
    WHERE codigo = p_codigo_vivienda;

    -- Sum all garajes participation percentages
    SELECT COALESCE(SUM(porcentaje_participacion), 0) INTO v_porcentaje_garajes
    FROM public.garajes
    WHERE unidad_familiar_codigo = p_codigo_vivienda;
...
AS $$
BEGIN
    -- Recalculate for all family units that use this vivienda
    PERFORM public.recalcular_participacion_unidad_familiar(NEW.codigo);
    RETURN NEW;
END;
$$;
