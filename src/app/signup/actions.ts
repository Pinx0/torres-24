...
async function checkRateLimit(
  ...
  if (rateLimit?.ultimo_intento) {
    ...
    const minutesSinceLastAttempt =
      (now.getTime() - lastAttempt.getTime()) / 60000;
  ...
  if (attempts >= MAX_ATTEMPTS) {
    const blockedUntil = new Date(
      now.getTime() + RATE_LIMIT_WINDOW_MINUTES * 60000,
    );
...
async function validatePhone(
  ...
  const { data, error } = await adminClient
    ...
    .select("telefono, apartamento, nombre, usado")
  ...
  if (error || !data) {
    return {
      valid: false,
      message:
        "El número de teléfono no está registrado en el sistema. Contacta con Pablo.",
    };
  }
  ...
  if (data.usado) {
    return {
      valid: false,
      message: "Este teléfono ya se ha usado para registrar una cuenta.",
    };
  }
...
export async function signUpWithPhone(formData: FormData) {
  ...
  if (!phoneRegex.test(phone)) {
    return {
      error:
        "El formato del teléfono no es válido. Debe incluir el código de país (ej: +34...)",
    };
  }
  ...
  if (!phoneValidation.valid) {
    return {
      error: phoneValidation.message || "El teléfono no está registrado",
    };
  }
  ...
  if (!apartamento) {
    return {
      error:
        "El teléfono no tiene un apartamento asociado. Contacta con Pablo.",
    };
  }
  ...
  if (viviendaError) {
    return {
      error:
        "El código de vivienda asociado al teléfono no existe en el sistema. Contacta con Pablo.",
    };
  }
  ...
  if (unidadError) {
    return {
      error:
        "La unidad familiar asociada al teléfono no existe en el sistema. Contacta con Pablo.",
    };
  }
  ...
  if (asociacionError) {
    console.error(
      "Error al asociar usuario con unidad familiar:",
      asociacionError,
    );
  ...
  if (otpSendError) {
    ...
    return {
      error:
        "Cuenta creada, pero no se pudo enviar el código de verificación. Por favor, intenta iniciar sesión y solicita un nuevo código.",
    };
  }

  const normalizedPhone = phone.trim().replace(/\s+/g, "");
  const { error: telefonoUsadoError } = await adminClient
    .from("telefonos_validos")
    .update({ usado: true })
    .eq("telefono", normalizedPhone)
    .eq("usado", false);

  if (telefonoUsadoError) {
    console.error("Error al marcar teléfono como usado:", telefonoUsadoError);
  }

  // Reset rate limit on successful signup
  await adminClient.from("limites_registro").delete().eq("correo", email);
